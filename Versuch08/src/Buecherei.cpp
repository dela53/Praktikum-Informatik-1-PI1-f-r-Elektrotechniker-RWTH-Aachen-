...
#define UNTERKLASSENVORHANDEN true // Aktivieren der Unterklassen
...
int main()
{
    ...
    do  // Anzeige des Menüs
    {
        std::cout << std::endl
				  ...
				  << "(6): Alle ausgeliehenen Medien anzeigen" << std::endl
                  << "(7): Beenden" << std::endl;
...
/////////////////////////////////////////////////////////////
// Funktion füllt die Datenbank der Buecherei automatisch //
/////////////////////////////////////////////////////////////
void fuelleDatenbank(std::vector<Medium*>& medien)
{
#if UNTERKLASSENVORHANDEN
...
void mediumEntfernen(std::vector<Medium*>& medien)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id) // @suppress("Method cannot be resolved")
        {
            delete (*it);  // Objekt löschen
            medien.erase(it); // Zeiger auf das Objekt löschen // @suppress("Invalid arguments")
    ...
    if(!success)
    {
        std::cerr << "Keine gültige ID!" << std::endl;
    }
}
...
void mediumAusleihen(std::vector<Medium*>& medien, Datum aktuellesDatum)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id) // @suppress("Method cannot be resolved")
        {
            (*it)->ausleihen(person,aktuellesDatum); // @suppress("Method cannot be resolved")
...
void mediumZurueckgeben(std::vector<Medium*>& medien)
{
    ...
    for(std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
    {
        if((*it)->getID() == id) // @suppress("Method cannot be resolved")
        {
            (*it)->zurueckgeben(); // @suppress("Method cannot be resolved")
            id_vorhanden = true;
        }
    }
    if (!id_vorhanden)
...
void alleMedienAusgeben(std::vector<Medium*>& medien)
{
    ...
    {
        std::cout << "*************************************************************" << std::endl;
        medien[index]->ausgabe(std::cout); // std::cout ist ein // @suppress("Method cannot be resolved")
    }
}
...
void alleAusgeliehenenMedienAusgeben(std::vector<Medium*>& medien)
{
	bool mindEinsAusgeliehen = false;

    std::cout << "Ausgeliehene Medien in der Bücherei:" << std::endl;

    for (unsigned int index = 0; index < medien.size(); index++)
    {
    	if (medien[index]->istAusgeliehen()) // sucht den index raus mit dem Status ausgeliehen // @suppress("Method cannot be resolved")
    	{
        std::cout << "*************************************************************" << std::endl;
        medien[index]->ausgabe(std::cout); // std::cout ist ein // @suppress("Method cannot be resolved")
        mindEinsAusgeliehen = true;
    	}
    }

    if (!mindEinsAusgeliehen)
    	{
